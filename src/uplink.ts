import {
  ...
  accountQuantity,
  accountUnit
} from '@ncp-labs/crypto-rate-utils'
...
export const setupHandlers = (
  ...
  plugin.registerDataHandler(async (data: Buffer) => {
    ...
    return hasConnectionTag
      ? // Connection ID exists in the ILP address, so route to Stream server (e.g. g.ncp.39hadn9ma.~n32j7ba)
        streamServerHandler(data)
      : // ILP address is for the root plugin, so route packet to sending connection (e.g. g.ncp.39hadn9ma)
